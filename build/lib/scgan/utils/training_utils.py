...
class ModelCheckpoint:
    ...
    def load(self, model, epoch, optimizers=None, schedulers=None, checkpoint_path=None):
        ...
        if checkpoint_path is None:
            ...
            if self.config.dataset == "Ru_KOS":
                load_filename = os.path.join(self.weight_dir, f'model_checkpoint_epoch_{epoch}.pth.tar')
            elif self.config.dataset == "Ru_Sber":
                load_filename = checkpoint_path
            else:
                load_filename = os.path.join(self.weight_dir, f'model_checkpoint_epoch_{epoch}')
        else:
            print("checkpoint_path is not None!")
            load_filename = checkpoint_path
        print(f"load_filename: {load_filename}")
